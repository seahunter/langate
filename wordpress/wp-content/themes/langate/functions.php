...
function langate_setup() {
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'langate_custom_background_args',
			array(
 ...
 */
function langate_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'langate_content_width', 640 );
}
add_action( 'after_setup_theme', 'langate_content_width', 0 );
...
function langate_scripts() {
	wp_enqueue_style( 'langate-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'langate-style', 'rtl', 'replace' );

	wp_enqueue_script( 'langate-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
